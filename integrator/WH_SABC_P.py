...
class WisdomHolman_SABC_P(object):
    '''
    Wisdom-Holman integrator for P(ABC)-type triple hierarchical systems.

    Stars A & B form an inner binary; star C orbits their COM; planet(s) orbit
    the COM of the entire triple (A+B+C) — circum-triple P-type orbits.

    Body order in HJS space: A, B, C, planet_0, ..., planet_{N-1}
    HJS coordinates (rows of transform matrix M):
      Row 0      : X_B    = r_B    - r_A
      Row 1      : X_C    = r_C    - COM(A + B)
      Row 2..N+1 : X_{Pj} = r_{Pj} - COM(A + B + C)
    ...
    def bind(self, particles, t0, tf, dt):
        ...
        # Collect planets (ptype == 1) — these orbit COM(A+B+C).
    ...
    def _build_transform_matrices(self):
        ...
        # Rows 2..N+1: X_{Pj} = r_{Pj} - COM(A+B+C)
        for j in range(N):
            M[j+2, 0]   = -self.mA / self.m_mid   # A (column 0)
            M[j+2, 1]   = -self.mB / self.m_mid   # B (column 1)
            M[j+2, 2]   = -self.mC / self.m_mid   # C (column 2)
            M[j+2, 3+j] =  1.0                    # +r_{Pj}
