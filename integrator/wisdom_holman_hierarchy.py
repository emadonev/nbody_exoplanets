...
class WisdomHolman_HJS(object):
    ...
    def bind(self, particles, t0, tf, dt):
        ...
        self.tf = float(tf)
        self.dt = float(dt)
        self.hierarchy = particles.tree_build()
    ...
    def propagate(self):
        ...
        x_jac = tools.cart2HJS(x_cart, self.particles.M_hjs, self.particles.masses, self.particles.N)

        # Drift-Kick-Drift (DKD)
        x_jac = self._drift_jacobi(x_jac, 0.5 * self.dt)
        x_cart_mid = tools.HJS2cart(x_jac, self.particles.M_hjs, self.particles.masses, self.particles.N)
        ...
        x_jac = self._drift_jacobi(x_jac, 0.5 * self.dt)
        x_cart_new = tools.HJS2cart(x_jac, self.particles.M_hjs, self.particles.masses, self.particles.N)
        self._set_active_cart(x_cart_new)


    def _refresh_active_order(self):
        p = self.particles
        active = p.orbit_nodes
        if active.size == 0:
